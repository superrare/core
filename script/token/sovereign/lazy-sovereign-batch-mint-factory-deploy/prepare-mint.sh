...
if [ -z "$FACTORY_ADDRESS" ]; then
    echo "Error: FACTORY_ADDRESS environment variable is required"
    echo "Set FACTORY_ADDRESS to your deployed LazySovereignBatchMintFactory address"
    exit 1
fi

if [ -z "$NUMBER_OF_TOKENS" ]; then
    echo "Error: NUMBER_OF_TOKENS environment variable is required"
    echo "Set NUMBER_OF_TOKENS to the number of tokens you want to prepare for minting"
    exit 1
fi
...
FORGE_CMD="forge script script/token/sovereign/lazy-sovereign-batch-mint-factory-deploy/prepare-mint.s.sol:PrepareMintScript --rpc-url ${RPC_URL} --via-ir  -vv"

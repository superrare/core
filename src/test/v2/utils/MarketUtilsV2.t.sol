...
contract TestContract {
    ...
    function addressMustHaveMarketplaceApprovedForNFT(address _addr, address _originContract, uint256 _tokenId)
        public
        view
    {
...
contract MarketUtilsV2Test is Test {
    ...
    function test_checkSplits_ZeroAddressRecipient() public {
        address payable[] memory splitAddrs = new address payable[](2);
        uint8[] memory splitRatios = new uint8[](2);

        splitAddrs[0] = payable(alice);
        splitAddrs[1] = payable(address(0));
        splitRatios[0] = 50;
        splitRatios[1] = 50;

        vm.expectRevert("checkSplits::Split cannot be zero address");
        tc.checkSplits(splitAddrs, splitRatios);
    }

    function test_checkSplits_ZeroRatio() public {
        address payable[] memory splitAddrs = new address payable[](2);
        uint8[] memory splitRatios = new uint8[](2);
        ...
        vm.expectRevert("checkSplits::Ratio must be greater than 0");
        tc.checkSplits(splitAddrs, splitRatios);
    }
    ...
    function test_refund_ERC20() public {
        ...
        vm.mockCall(
            address(rare), abi.encodeWithSelector(IERC20.transfer.selector, alice, totalAmount), abi.encode(true)
        );
    ...
    function test_payout_primary() public {
        ...
        vm.mockCall(
            ...
            abi.encodeWithSelector(
                IMarketplaceSettings.getERC721ContractPrimarySaleFeePercentage.selector, originContract
            ),
            abi.encode(15)
        );
        ...
        tc.payout{value: amount + ((amount * 3) / 100)}(
            originContract, tokenId, currencyAddress, amount, charlie, splitAddrs, splitRatios
        );
    ...
    function test_payout_tooManyRoyaltyRecipients() public {
        ...
        assertEq(
            aliceBalanceAfter, aliceBalanceBefore + expectedRoyaltyPerRecipient, "First recipient should receive 1%"
        );
    ...
    function test_payout_truncationVerifyOrder() public {
        ...
        assertEq(
            address(0x1111).balance, addr1111BalanceBefore + ((amount * 3) / 100), "Third recipient should receive 3%"
        );
        assertEq(
            address(0x2222).balance, addr2222BalanceBefore + ((amount * 4) / 100), "Fourth recipient should receive 4%"
        );
        assertEq(
            address(0x3333).balance, addr3333BalanceBefore + ((amount * 5) / 100), "Fifth recipient should receive 5%"
        );
    ...
    function test_payout_primary_noRoyalties() public {
        ...
        assertEq(
            aliceBalanceAfter, aliceExpectedBalance, "royalty receiver should not receive anything in primary sale"
        );
    }

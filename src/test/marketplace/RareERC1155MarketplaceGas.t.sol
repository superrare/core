...
contract CheckoutGasRoyaltyEngine is IRoyaltyEngineV1 {
    ...
    function getRoyalty(address, uint256, uint256 _value)
        external
        view
    ...
    function getRoyaltyView(address, uint256, uint256 _value)
        external
        view
        returns (address payable[] memory recipients, uint256[] memory amounts)
    {
    ...
    function _royalties(uint256 _value)
        private
        view
        returns (address payable[] memory recipients, uint256[] memory amounts)
    {
...
contract RareERC1155MarketplaceGasTest is Test {
    ...
    function testGas_checkoutSecondaryEthMaxSplitsAndRoyalties_sweep() public {
        ...
        for (uint256 i = 0; i < counts.length; i++) {
            ...
            IRareERC1155MarketplaceTypes.CheckoutItem[] memory items =
                _prepareSecondaryCheckoutItems(count, address(0), PRICE);
            _measureCheckout(
                "checkout_secondary_eth_max_splits_max_royalties", count, items, _withFee(PRICE) * count, count, 0
            );
        }
    }
    ...
    function testGas_checkoutSecondaryErc20MaxSplitsAndRoyalties_sweep() public {
        ...
        for (uint256 i = 0; i < counts.length; i++) {
            ...
            _deployFixture(MAX_ROYALTY_RECIPIENTS);
            IRareERC1155MarketplaceTypes.CheckoutItem[] memory items =
                _prepareSecondaryCheckoutItems(count, address(currency), PRICE);
    ...
    function testGas_checkoutMixedPrimaryAndSecondaryEth_sweep() public {
        ...
        for (uint256 i = 0; i < counts.length; i++) {
            ...
            IRareERC1155MarketplaceTypes.CheckoutItem[] memory secondaryItems =
                _prepareSecondaryCheckoutItems(secondaryCount, address(0), PRICE);
            IRareERC1155MarketplaceTypes.CheckoutItem[] memory items =
                _concatCheckoutItems(primaryItems, secondaryItems);
    ...
    function testGas_checkoutSkippedSecondaryStaleBalanceThenPrimaryEth_sweep() public {
        ...
        for (uint256 i = 0; i < counts.length; i++) {
            ...
            IRareERC1155MarketplaceTypes.CheckoutItem[] memory skippedItems =
                _prepareStaleBalanceSecondaryCheckoutItems(skippedCount, address(0));
    ...
    function _preparePrimaryCheckoutItems(uint256 _count)
        private
        returns (IRareERC1155MarketplaceTypes.CheckoutItem[] memory items)
    {
    ...
    function _prepareSecondaryCheckoutItems(uint256 _count, address _currencyAddress, uint256 _itemPrice)
        private
        returns (IRareERC1155MarketplaceTypes.CheckoutItem[] memory items)
    {
    ...
    function _prepareStaleBalanceSecondaryCheckoutItems(uint256 _count, address _currencyAddress)
        private
        returns (IRareERC1155MarketplaceTypes.CheckoutItem[] memory items)
    {
        uint256[] memory tokenIds = _prepareSecondaryListings(_count, _currencyAddress);
    ...
    function _prepareSecondaryListings(uint256 _count, address _currencyAddress)
        private
    ...
    function _directSaleCheckoutItem(uint256 _tokenId, uint256 _price)
        private
        view
        returns (IRareERC1155MarketplaceTypes.CheckoutItem memory)
    {
    ...
    function _listingCheckoutItem(uint256 _tokenId, address _currencyAddress, uint256 _price)
        private
        view
        returns (IRareERC1155MarketplaceTypes.CheckoutItem memory)
    {
    ...
    function _directSaleRequests(uint256[] memory _tokenIds, uint256 _price)
        private
        pure
        returns (IRareERC1155MarketplaceTypes.DirectSaleRequest[] memory requests)
    {
    ...
    function _salePriceRequests(uint256[] memory _tokenIds, uint256 _price)
        private
        pure
        returns (IRareERC1155MarketplaceTypes.SalePriceRequest[] memory requests)
    {
    ...
    function _mintRequests(uint256[] memory _tokenIds, uint256 _price)
        private
        pure
        returns (IRareERC1155MarketplaceTypes.MintRequest[] memory requests)
    {
    ...
    function _buyRequests(uint256[] memory _tokenIds, uint256 _price)
        private
        pure
        returns (IRareERC1155MarketplaceTypes.BuyRequest[] memory requests)
    {

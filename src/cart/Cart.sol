  ...
  CartStorage
{
  ...
  uint256 private constant MAX_ORDER_LINES = 32;
  ...
  function _executePurchaseCore(
    ...
    (uint256[] memory lineListing, bytes32[] memory resolvedDigests) = _resolveRootListings(
      lines,
      listings,
      authorization
    );
    // Check listing terms, calculate requested quantities, and check available quantities.
    uint256[] memory listingRequested = _validateLineTerms(
    ...
    for (uint256 i = 0; i < listings.length; ++i) {
      ...
      if (
        !MerkleProof.verifyCalldata(
          authorization.listingProofs[i],
          root.listingsRoot,
          CartHashing.hashListingLeaf(digest)
        )
      ) {
  ...
  function _validateListing(Listing calldata listing) private view {
    ...
    if (
      listing.fulfillmentKind == FulfillmentKind.ERC721_TRANSFER &&
  ...
  function _validateLineTerms(
    ...
    for (uint256 i = 0; i < lines.length; ++i) {
      ...
      if (listingIndex == NO_LISTING) {
        if (line.fulfillmentKind != FulfillmentKind.NONE && line.fulfillmentKind != FulfillmentKind.CURRENCY_SWAP) {
      ...
      if (
        line.fulfillmentKind != listing.fulfillmentKind ||
  ...
  function _beginPayment(
    PurchaseOrder calldata order,
    OrderLine[] calldata lines,
    PayoutRoute memory route
  ) private returns (ICartPayments.PaymentState memory state) {
    (bool success, bytes memory result) = address(PAYMENT_EXECUTOR).delegatecall(
      abi.encodeCall(ICartPayments.begin, (order, lines, route))
    );
  ...
  function _finishPayment(
    ...
    (bool success, bytes memory result) = address(PAYMENT_EXECUTOR).delegatecall(
      abi.encodeCall(ICartPayments.finish, (orderId, paymentCurrency, lines, state))
    );
    if (!success) _bubble(result);
  }
  ...
  function _isOnChainKind(FulfillmentKind kind) private pure returns (bool) {
    ...
    return
      kind == FulfillmentKind.ERC721_TRANSFER ||
      kind == FulfillmentKind.ERC1155_TRANSFER ||
      kind == FulfillmentKind.ERC721_MINT_TO ||
      kind == FulfillmentKind.ERC1155_MINT_TO;
  }

// contracts/token/ERC721/sovereign/LazySovereignBatchMint.sol
...
contract LazySovereignBatchMint is
  OwnableUpgradeable,
  ERC165Upgradeable,
  ERC721Upgradeable,
  ITokenCreator,
  ...
  ERC2981Upgradeable
{
  ...
  event LazyTokenCancelled(uint256 indexed tokenId, uint256 indexed batchIndex);
  ...
  function prepareMint(string calldata _baseURI, uint256 _numberOfTokens) public onlyOwner ifNotDisabled {
    require(mintBatches.length < MAX_BATCHES, "prepareMint::exceeded max batches");
    require(_numberOfTokens > 0, "prepareMint::numberOfTokens must be greater than 0");
    ...
    require(endTokenId <= maxTokens, "prepareMint::exceeded maxTokens");

    mintBatches.push(MintBatch(startTokenId, endTokenId, _baseURI));

    emit PrepareMint(startTokenId, endTokenId, _baseURI);
  }
  ...
  function burn(uint256 _tokenId) public virtual override {
    (bool isLazyToken, uint256 batchIndex, ) = _batchMintInfo(_tokenId);

    if (isLazyToken && !ERC721Upgradeable._exists(_tokenId)) {
      // Lazy token that was never minted - only owner can "burn" (cancel) it
      require(msg.sender == owner(), "burn::only owner can cancel unminted lazy token");
      tokensBurned[_tokenId] = true;
      emit LazyTokenCancelled(_tokenId, batchIndex);
      return;
    }

    // Token exists - standard burn flow
    // Clear any existing approval to prevent stale approvals if token IDs are reused
    _tokenApprovals[_tokenId] = address(0);
    totalSupplyCounter.decrement();
    // Call parent burn (which checks ownership via _isApprovedOrOwner)
    ERC721BurnableUpgradeable.burn(_tokenId);
    // Mark as burned after parent burn completes (parent burn deletes token, but we track burned state for lazy tokens)
    tokensBurned[_tokenId] = true;
  }
  ...
  function ownerOf(uint256 _tokenId) public view virtual override returns (address) {
    (bool isLazyToken, , ) = _batchMintInfo(_tokenId);

    // Regular tokens
    if (!isLazyToken) {
      // Will revert if token truly does not exist
  ...
  function _isApprovedOrOwner(address _spender, uint256 _tokenId) internal view virtual override returns (bool) {
    ...
    // Handle unminted lazy tokens - owner can transfer them
    if (isLazyToken && !ERC721Upgradeable._exists(_tokenId) && !tokensBurned[_tokenId]) {
      // For unminted lazy tokens, check if spender is owner or approved for all
      return (_spender == owner() || isApprovedForAll(owner(), _spender));
    }

    // For minted tokens, use standard ERC721 logic
  ...
  function _approve(address to, uint256 tokenId) internal override {
    ...
    // For unminted lazy tokens, use owner() as the owner for the Approval event
    (bool isLazyToken, , ) = _batchMintInfo(tokenId);
    address tokenOwner = (isLazyToken && !ERC721Upgradeable._exists(tokenId) && !tokensBurned[tokenId])
      ? owner()
      : ownerOf(tokenId);
  ...
  function getApproved(uint256 _tokenId) public view virtual override returns (address) {
    ...
    bool exists;
    if (isLazyToken) {
      // Lazy token exists only if actually minted
      exists = ERC721Upgradeable._exists(_tokenId) && !tokensBurned[_tokenId];
    } else {
      // Regular token
      exists = ERC721Upgradeable._exists(_tokenId);
    }

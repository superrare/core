...
library MarketUtilsV2 {
    ...
    function addressMustHaveMarketplaceApprovedForNFT(
        ...
        require(
            nft.isApprovedForAll(_address, address(_config.erc721ApprovalManager)), "owner must have approved token"
        );
    }
    ...
    function checkSplits(address payable[] calldata _splitAddrs, uint8[] calldata _splitRatios) internal pure {
        ...
        for (uint256 i = 0; i < _splitRatios.length; i++) {
            require(_splitAddrs[i] != address(0), "checkSplits::Split cannot be zero address");
            require(_splitRatios[i] > 0, "checkSplits::Ratio must be greater than 0");
        ...
        if (!_config.marketplaceSettings.hasERC721TokenSold(_originContract, _tokenId)) {
            ...
            if (_config.spaceOperatorRegistry.isApprovedSpaceOperator(_seller)) {
                ...
                uint256 platformCommission =
                    _config.marketplaceSettings.getERC721ContractPrimarySaleFeePercentage(_originContract);
            ...
            (address payable[] memory recipients, uint256[] memory amounts) =
                _config.royaltyEngine.getRoyalty(_originContract, _tokenId, _amount);

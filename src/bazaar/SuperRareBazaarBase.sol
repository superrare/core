...
abstract contract SuperRareBazaarBase is SuperRareBazaarStorage {
    ...
    function _checkSplits(address payable[] calldata _splits, uint8[] calldata _ratios) internal pure {
        ...
        for (uint256 i = 0; i < _ratios.length; i++) {
            require(_splits[i] != address(0), "checkSplits::Split cannot be zero address");
            require(_ratios[i] > 0, "checkSplits::Ratio must be greater than 0");
    ...
    function _senderMustHaveMarketplaceApproved(address _contract, uint256 _amount) internal view {
        ...
        require(
            erc20.allowance(msg.sender, address(this)) >= _amount, "sender needs to approve marketplace for currency"
        );
    }
    ...
    function _refund(address _currencyAddress, uint256 _amount, uint256 _marketplaceFee, address _recipient) internal {
        if (_amount == 0) {
            return;
        }
        ...
        if (!marketplaceSettings.hasERC721TokenSold(_originContract, _tokenId)) {
            ...
            if (spaceOperatorRegistry.isApprovedSpaceOperator(_seller)) {
                ...
                uint256 platformCommission =
                    marketplaceSettings.getERC721ContractPrimarySaleFeePercentage(_originContract);

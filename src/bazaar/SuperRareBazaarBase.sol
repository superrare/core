...
abstract contract SuperRareBazaarBase is SuperRareBazaarStorage {
  ...
  /// @dev Flow: 1) Royalty, 2) App fee (if _app and appRegistry set), 3) Seller splits.
  /// @dev Protocol share of app fee goes to networkBeneficiary.
  ...
  /// @param _app App that facilitated the trade; address(0) = no app fee.
  function _payout(
    ...
    // 2. App fee (if any) — registry calculates amounts; Bazaar routes protocol share to networkBeneficiary
    address effectiveApp = (appRegistry != address(0)) ? _app : address(0);
    if (effectiveApp != address(0)) {
      (uint256 appShare, uint256 protocolShare, uint256 totalFee) =
        IRareAppRegistry(appRegistry).calculateFeeSplit(effectiveApp, remainingAmount);

      if (totalFee > 0) {
        remainingAmount -= totalFee;

        (, , address appFeeRecipient) = IRareAppRegistry(appRegistry).apps(effectiveApp);

        if (appShare > 0) {
          address payable[] memory appRecip = new address payable[](1);
          uint256[] memory appAmts = new uint256[](1);
          appRecip[0] = payable(appFeeRecipient);
          appAmts[0] = appShare;
          _performPayouts(_currencyAddress, appShare, appRecip, appAmts);
        }

        if (protocolShare > 0) {
          address payable[] memory protocolRecip = new address payable[](1);
          uint256[] memory protocolAmts = new uint256[](1);
          protocolRecip[0] = payable(networkBeneficiary);
          protocolAmts[0] = protocolShare;
          _performPayouts(_currencyAddress, protocolShare, protocolRecip, protocolAmts);
        }
      }
    }
